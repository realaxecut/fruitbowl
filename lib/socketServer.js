...
// Ensure the discord_links table exists (idempotent)
db.exec(`
...
`);
const stmtGetDiscord = db.prepare('SELECT discord_id, in_guild FROM discord_links WHERE wallet = ?');
...
/** Returns true if wallet has linked Discord AND is in the required guild. */
async function isDiscordMember(wallet) {
  if (!wallet) return false;
  const row = stmtGetDiscord.get(wallet);
  if (!row) return false;                     // not linked
  if (!DISCORD_BOT_TOKEN || !DISCORD_GUILD_ID) return !!row.in_guild; // no bot, trust stored value

  // Re-verify with Discord API (lightweight, cached on the bot side)
  try {
    ...
    stmtUpdateGuild.run({ wallet, in_guild: ok ? 1 : 0, now: Date.now() });
    return ok;
  } catch {
    return !!row.in_guild; // fall back to cached value on network error
  }
}
// ─────────────────────────────────────────────────────────────────────────
...
// ── Slice Duel in-memory stores ───────────────────────────────────────────
const sliceDuelLobbies = new Map(); // lobbyId -> lobby
const sliceDuelMatches = new Map(); // matchId -> match
...
function initSocketServer(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    // ── Slice Duel handlers ──────────────────────────────────────────────────
    socket.on('sliceduel_get_lobbies', () => {
      const open = Array.from(sliceDuelLobbies.values()).filter(l => l.status === 'open');
      socket.emit('sliceduel_lobbies', open);
    });

    socket.on('sliceduel_create_lobby', ({ wallet, displayName, wagerLamports, txSignature }) => {
      if (!wallet || !wagerLamports) return;
      const testCashRow = stmts.getModSetting.get('mod_sliceduel_test_cash');
      const isTestCash = testCashRow?.value === '1';
      for (const [id, lobby] of sliceDuelLobbies.entries()) {
        if (lobby.creatorWallet === wallet && lobby.status === 'open') sliceDuelLobbies.delete(id);
      }
      const { v4: uuidv4sd } = require('uuid');
      const lobbyId = uuidv4sd();
      const lobby = { id: lobbyId, creatorWallet: wallet, creatorName: displayName || wallet.slice(0,8), wagerLamports, txSignature: isTestCash ? 'TEST_CASH' : txSignature, createdAt: Date.now(), status: 'open', creatorSocketId: socket.id, isTestCash };
      sliceDuelLobbies.set(lobbyId, lobby);
      socket.join('sliceduel_lobby_' + lobbyId);
      socket.emit('sliceduel_lobby_created', lobby);
      io.emit('sliceduel_lobbies', Array.from(sliceDuelLobbies.values()).filter(l => l.status === 'open'));
      setTimeout(() => {
        const l = sliceDuelLobbies.get(lobbyId);
        if (l && l.status === 'open') {
          sliceDuelLobbies.delete(lobbyId);
          io.to('sliceduel_lobby_' + lobbyId).emit('sliceduel_lobby_expired');
          io.emit('sliceduel_lobbies', Array.from(sliceDuelLobbies.values()).filter(l => l.status === 'open'));
        }
      }, 5 * 60 * 1000);
      console.log('[sliceduel] Lobby created by ' + wallet + (isTestCash ? ' [TEST CASH]' : ''));
    });

    socket.on('sliceduel_cancel_lobby', ({ lobbyId, wallet }) => {
      const lobby = sliceDuelLobbies.get(lobbyId);
      if (!lobby || lobby.creatorWallet !== wallet) return;
      sliceDuelLobbies.delete(lobbyId);
      io.emit('sliceduel_lobbies', Array.from(sliceDuelLobbies.values()).filter(l => l.status === 'open'));
    });

    socket.on('sliceduel_join_lobby', ({ lobbyId, wallet, displayName, txSignature }) => {
      const lobby = sliceDuelLobbies.get(lobbyId);
      if (!lobby || lobby.status !== 'open' || lobby.creatorWallet === wallet) {
        socket.emit('sliceduel_join_error', { error: 'Lobby not available' }); return;
      }
      lobby.status = 'matched';
      lobby.opponentWallet = wallet;
      lobby.opponentName = displayName || wallet.slice(0,8);
      lobby.opponentSocketId = socket.id;
      lobby.opponentTxSig = txSignature;
      const { v4: uuidv4sd2 } = require('uuid');
      const matchId = uuidv4sd2();
      const seed = Math.floor(Math.random() * 2147483647);
      const match = { id: matchId, lobbyId, creatorWallet: lobby.creatorWallet, opponentWallet: wallet, wagerLamports: lobby.wagerLamports, seed, scores: {}, ended: false, endedCount: 0, isTestCash: !!lobby.isTestCash };
      match.scores[lobby.creatorWallet] = 0;
      match.scores[wallet] = 0;
      sliceDuelMatches.set(matchId, match);
      sliceDuelLobbies.set(lobbyId, Object.assign({}, lobby, { matchId }));
      socket.join('sliceduel_lobby_' + lobbyId);
      const matchPayload = { lobby: Object.assign({}, lobby, { opponentWallet: wallet, opponentName: displayName }), seed, matchId };
      io.to('sliceduel_lobby_' + lobbyId).emit('sliceduel_match_found', matchPayload);
      io.emit('sliceduel_lobbies', Array.from(sliceDuelLobbies.values()).filter(l => l.status === 'open'));
      console.log('[sliceduel] Match ' + matchId + ': ' + lobby.creatorWallet + ' vs ' + wallet);
    });

    socket.on('sliceduel_score_update', ({ matchId, wallet, score }) => {
      const match = sliceDuelMatches.get(matchId);
      if (!match || match.ended) return;
      match.scores[wallet] = score || 0;
    });

    socket.on('sliceduel_game_end', ({ matchId, wallet, score }) => {
      const match = sliceDuelMatches.get(matchId);
      if (!match) return;
      match.scores[wallet] = score || 0;
      match.endedCount = (match.endedCount || 0) + 1;
      const settle = () => {
        if (match.ended) return;
        match.ended = true;
        const cScore = match.scores[match.creatorWallet] || 0;
        const oScore = match.scores[match.opponentWallet] || 0;
        const winnerWallet = cScore >= oScore ? match.creatorWallet : match.opponentWallet;
        const payoutLamports = Math.floor(match.wagerLamports * 2 * 0.95);
        if (match.isTestCash) {
          console.log('[sliceduel] Test cash match ' + matchId + ' settled. Winner: ' + winnerWallet + ' (' + cScore + ' vs ' + oScore + ') [NO PAYOUT — TEST CASH]');
        } else {
          try {
            const { db } = require('./db.js');
            const { v4: uuidv4sd3 } = require('uuid');
            const claimId = uuidv4sd3();
            db.prepare('INSERT OR IGNORE INTO pending_claims (round_id, wallet, amount, claimed, created_at) VALUES (?, ?, ?, 0, ?)').run(claimId, winnerWallet, payoutLamports, Date.now());
            console.log('[sliceduel] Match ' + matchId + ' settled. Winner: ' + winnerWallet + ' (' + cScore + ' vs ' + oScore + ')');
          } catch(e) { console.error('[sliceduel] DB error:', e); }
        }
        io.to('sliceduel_lobby_' + match.lobbyId).emit('sliceduel_result', { winnerWallet, yourScore: cScore, opponentScore: oScore, payoutLamports: match.isTestCash ? 0 : payoutLamports, isTestCash: !!match.isTestCash });
      };
      if (match.endedCount >= 2) { settle(); } else { setTimeout(settle, 6000); }
    });

    socket.on('sliceduel_claim_payout', ({ matchId, wallet }) => {
      socket.emit('sliceduel_claim_result', { success: false, error: 'Use Settings → Unclaimed Winnings to claim.' });
    });
    ...
    // ── Daily crate (server-authoritative cooldown) ────────────────────────
    socket.on('get_crate_state', ({ wallet }) => {
      if (!wallet) return;
      const CRATE_COOLDOWN_MS = 24 * 60 * 60 * 1000;
      const row = stmts.getCrateCooldown.get(wallet);
      if (!row) {
        socket.emit('crate_state', { available: true, nextAt: 0 });
        return;
      }
      const nextAt = row.last_opened + CRATE_COOLDOWN_MS;
      socket.emit('crate_state', { available: Date.now() >= nextAt, nextAt });
    });

    socket.on('open_daily_crate', async ({ wallet, prizes }) => {
      if (!wallet) { socket.emit('crate_result', { success: false, error: 'Not connected' }); return; }

      // Discord guild gate
      const crateDiscordOk = await isDiscordMember(wallet);
      if (!crateDiscordOk) {
        const discordRow = stmtGetDiscord.get(wallet);
        socket.emit('crate_result', { success: false, error: 'discord_required', linked: !!discordRow });
        return;
      }
      ...
      if (row) {
        const nextAt = row.last_opened + CRATE_COOLDOWN_MS;
        if (Date.now() < nextAt) {
          socket.emit('crate_result', { success: false, error: 'Crate not yet available', nextAt });
          return;
        }
      }

      // Roll the prize server-side
      // If xp_only mode is on, always force index 0 (the XP consolation prize)
      const xpOnly = stmts.getModSetting.get('mod_fruitflip_xp_only');
      let prizeIdx = 0;
      if (xpOnly?.value !== '1') {
      ...
      stmts.setCrateCooldown.run({ wallet, last_opened: now, now });
      console.log(`[crate] ${wallet} opened crate — prize: ${prize.label}`);
      socket.emit('crate_result', { success: true, prizeIdx, prize, nextAt: now + CRATE_COOLDOWN_MS });
    });

    // ── Mod: always_lose and fruitflip flags (server-side) ──────────────────
    socket.on('mod_set_fruitroll_always_lose', ({ wallet, value }) => {
      if (!isModerator(wallet)) { socket.emit('mod_error', { error: 'Not authorized' }); return; }
      stmts.setModSetting.run({ key: 'mod_fruitroll_always_lose', value: value ? '1' : '0', now: Date.now() });
      socket.emit('mod_fruitroll_always_lose_result', { success: true, value: !!value });
      // Re-emit full flags so fruitroll page state syncs
      const propMoney = stmts.getModSetting.get('mod_fruitflip_prop_money');
      const xpOnly = stmts.getModSetting.get('mod_fruitflip_xp_only');
      socket.emit('mod_fruitroll_flags', { alwaysLose: !!value, propMoney: propMoney?.value === '1', xpOnly: xpOnly?.value === '1' });
      console.log(`[mod] fruitroll_always_lose set to ${value} by ${wallet}`);
    });

    socket.on('mod_get_fruitroll_always_lose', ({ wallet }) => {
      if (!isModerator(wallet)) return;
      const row = stmts.getModSetting.get('mod_fruitroll_always_lose');
      socket.emit('mod_fruitroll_always_lose_result', { success: true, value: row?.value === '1' });
    });

    socket.on('mod_set_fruitflip_prop_money', ({ wallet, value }) => {
      if (!isModerator(wallet)) { socket.emit('mod_error', { error: 'Not authorized' }); return; }
      stmts.setModSetting.run({ key: 'mod_fruitflip_prop_money', value: value ? '1' : '0', now: Date.now() });
      socket.emit('mod_fruitflip_prop_money_result', { success: true, value: !!value });
      // Re-emit full flags so fruitroll page state syncs
      const alwaysLose = stmts.getModSetting.get('mod_fruitroll_always_lose');
      const xpOnly = stmts.getModSetting.get('mod_fruitflip_xp_only');
      socket.emit('mod_fruitroll_flags', { alwaysLose: alwaysLose?.value === '1', propMoney: !!value, xpOnly: xpOnly?.value === '1' });
    });

    socket.on('mod_set_fruitflip_xp_only', ({ wallet, value }) => {
      if (!isModerator(wallet)) { socket.emit('mod_error', { error: 'Not authorized' }); return; }
      stmts.setModSetting.run({ key: 'mod_fruitflip_xp_only', value: value ? '1' : '0', now: Date.now() });
      socket.emit('mod_fruitflip_xp_only_result', { success: true, value: !!value });
      // Re-emit full flags so fruitroll page state syncs
      const alwaysLose = stmts.getModSetting.get('mod_fruitroll_always_lose');
      const propMoney = stmts.getModSetting.get('mod_fruitflip_prop_money');
      socket.emit('mod_fruitroll_flags', { alwaysLose: alwaysLose?.value === '1', propMoney: propMoney?.value === '1', xpOnly: !!value });
    });

    socket.on('get_mod_fruitroll_flags', ({ wallet }) => {
      if (!isModerator(wallet)) return;
      const alwaysLose = stmts.getModSetting.get('mod_fruitroll_always_lose');
      const propMoney = stmts.getModSetting.get('mod_fruitflip_prop_money');
      const xpOnly = stmts.getModSetting.get('mod_fruitflip_xp_only');
      const testCash = stmts.getModSetting.get('mod_sliceduel_test_cash');
      socket.emit('mod_fruitroll_flags', {
        alwaysLose: alwaysLose?.value === '1',
        propMoney: propMoney?.value === '1',
        xpOnly: xpOnly?.value === '1',
        sliceDuelTestCash: testCash?.value === '1',
      });
    });

    socket.on('mod_set_sliceduel_test_cash', ({ wallet, value }) => {
      if (!isModerator(wallet)) { socket.emit('mod_error', { error: 'Not authorized' }); return; }
      stmts.setModSetting.run({ key: 'mod_sliceduel_test_cash', value: value ? '1' : '0', now: Date.now() });
      const alwaysLose = stmts.getModSetting.get('mod_fruitroll_always_lose');
      const propMoney = stmts.getModSetting.get('mod_fruitflip_prop_money');
      const xpOnly = stmts.getModSetting.get('mod_fruitflip_xp_only');
      socket.emit('mod_fruitroll_flags', {
        ...
        propMoney: propMoney?.value === '1',
        xpOnly: xpOnly?.value === '1',
        sliceDuelTestCash: !!value,
      });
      console.log(`[mod] sliceduel_test_cash set to ${value} by ${wallet}`);
    });
  });
