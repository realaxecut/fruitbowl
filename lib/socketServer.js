  ...
  setTimeout(async () => {
    ...
    if (spun.winnerWallet && spun.winnerShare > 0) {
      ...
      if (payoutTx) {
        ...
        recordWin(spun.winnerWallet, spun.winnerShare);

        // Check if winner was referred — pay 30% bonus to referrer
        const referrerWallet = getReferrer(spun.winnerWallet);
        if (referrerWallet) {
          const bonusAmount = recordReferralBonus(referrerWallet, spun.winnerWallet, spun.id, spun.winnerShare);
          if (bonusAmount && bonusAmount > 0) {
            const referralTx = await sendPayout(referrerWallet, bonusAmount);
            console.log(`[referral] Paid ${bonusAmount} lamports bonus to ${referrerWallet} (referred ${spun.winnerWallet}) — tx: ${referralTx}`);
            // Notify the referrer if they're connected
            io.emit('referral_bonus', {
              referrerWallet,
              referredWallet: spun.winnerWallet,
              bonusAmount,
              referralTx,
            });
          }
        ...
        console.error('[payout] Payout failed for winner', spun.winnerWallet, '— NOT recording win');
      }
    }
