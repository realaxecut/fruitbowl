...
function initSocketServer(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('sliceduel_create_lobby', ({ wallet, displayName, wagerLamports, txSignature }) => {
      ...
      const lobby = { id: lobbyId, creatorWallet: wallet, creatorName: displayName || wallet.slice(0,8), wagerLamports, txSignature, createdAt: Date.now(), status: 'open', creatorSocketId: socket.id };
      ...
      console.log('[sliceduel] Lobby created by ' + wallet);
    });
    ...
    socket.on('sliceduel_join_lobby', ({ lobbyId, wallet, displayName, txSignature }) => {
      ...
      const match = { id: matchId, lobbyId, creatorWallet: lobby.creatorWallet, opponentWallet: wallet, wagerLamports: lobby.wagerLamports, seed, scores: {}, ended: false, endedCount: 0 };
    ...
    socket.on('sliceduel_game_end', ({ matchId, wallet, score }) => {
      ...
      const settle = () => {
        ...
        io.to('sliceduel_lobby_' + match.lobbyId).emit('sliceduel_result', { winnerWallet, yourScore: cScore, opponentScore: oScore, payoutLamports });
      };
