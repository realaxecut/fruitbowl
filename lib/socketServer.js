...
function initSocketServer(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('claim_payout', async ({ wallet, roundId }) => {
      ...
      if (referrerWallet) {
        const bonusAmount = recordReferralBonus(referrerWallet, wallet, roundId, claim.amount);
        if (bonusAmount && bonusAmount > 0) {
          const referralTx = await sendPayout(referrerWallet, bonusAmount);
          console.log(`[referral] Paid ${bonusAmount} lamports bonus to ${referrerWallet} — tx: ${referralTx}`);
          io.emit('referral_bonus', { referrerWallet, referredWallet: wallet, bonusAmount, referralTx });
        }
      }
    ...
    socket.on('claim_fruitroll_payout', async ({ wallet, claimId }) => {
      ...
      const referrerWallet = getReferrer(wallet);
      if (referrerWallet) {
        const bonusAmount = recordReferralBonus(referrerWallet, wallet, claimId, claim.payout_lamports);
        if (bonusAmount && bonusAmount > 0) {
          const referralTx = await sendPayout(referrerWallet, bonusAmount);
          console.log(`[referral] Paid ${bonusAmount} lamports bonus to ${referrerWallet} — tx: ${referralTx}`);
          io.emit('referral_bonus', { referrerWallet, referredWallet: wallet, bonusAmount, referralTx });
