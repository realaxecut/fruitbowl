...
function recordReferralBonus(referrerWallet, referredWallet, roundId, winAmount) {
  const bonusAmount = Math.floor(winAmount * REFERRAL_BONUS_RATE);
  ...
  // Credit total_won for the referrer
  stmts.updateStats.run({ bet: 0, won: bonusAmount, now: Date.now(), wallet: referrerWallet });
...
module.exports = {
  ...
  isModerator, muteUser, deleteMessage, deleteChatByWallet, clearAllChat,
